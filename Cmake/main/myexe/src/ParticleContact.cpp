...
ParticleContact::ParticleContact(Particle* p_Particles[2], double p_CoefficientRestitution,
                                 Vector3D p_ContactNormale, double p_Penetration)
{
...
void ParticleContact::ResolveVelocity(float p_Duration)
{
    ...
    m_Particles[0]->setVelocity(
        m_ContactNormale.mulScalaireResult(m_Particles[0]->getInverseMasse() * l_VSP * p_Duration));
    m_Particles[0]->setVelocity(
        m_ContactNormale.mulScalaireResult(m_Particles[0]->getInverseMasse() * l_VSP * p_Duration));
}
...
{
    Vector3D l_DistanceParticle0 = m_ContactNormale.mulScalaireResult(
        (m_Particles[1]->getInverseMasse() /
         (m_Particles[0]->getInverseMasse() - m_Particles[1]->getInverseMasse())) *
        m_Penetration);
    Vector3D l_DistanceParticle1 = m_ContactNormale.mulScalaireResult(
        -(m_Particles[0]->getInverseMasse() /
          (m_Particles[0]->getInverseMasse() - m_Particles[1]->getInverseMasse())) *
        m_Penetration);
