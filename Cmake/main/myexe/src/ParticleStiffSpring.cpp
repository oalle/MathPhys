...
ParticleStiffSpring::ParticleStiffSpring(Vector3D p_AnchoredPoint, double p_Damping,
...
void ParticleStiffSpring::updateForce(Particle* p_Particle, float p_Duration)
{
    ...
    Vector3D l_accelResultante =
        p_Particle->getPosition().mulScalaireResult(-this->m_ConstantSpring) -
        p_Particle->getVelocity().mulScalaireResult(this->m_Damping);
